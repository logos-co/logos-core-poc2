...
import std/[os, strutils, dynlib], results, ./abi_types

{.pragma: capi, cdecl, raises: [], gcsafe.}

type SharedModule* = object
  name*: string
  version*: string
  schema*: string
  handle*: LibHandle
...
proc init*(_: type SharedModule, path: string): Result[SharedModule, string] =
  ...
  let freeSym = dynlib.symAddr(handle, cstring(modulePrefix & "free"))

  # Check mandatory symbols
  if dispatchSym.isNil or freeSym.isNil:
    dynlib.unloadLib(handle)
    return err(
      "Missing mandatory module symbols: logos_" & moduleName &
        "_dispatch and/or logos_free"
    )
